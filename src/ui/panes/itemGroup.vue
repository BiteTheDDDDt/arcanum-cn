...
<template>
	<div v-if="shown.length > 0">
		...
		<div v-if="isOpen">
			...
			<div class="rsrc separate hidable" v-for="it in shown" :data-key="it.id" :key="it.id"
				@mouseenter.capture.stop="itemOver($event, it)">

				<span class="item-name">{{ it.name }}</span>
				<span class="num-align">{{ formatNumber(it.value.value > 0 ? Math.floor(it.value.value) : 0) + (it.max && it.max.value > 0 ? '/' +
					formatNumber(it.max.value) : '') }}</span>
