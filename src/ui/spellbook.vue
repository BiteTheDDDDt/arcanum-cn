...
export default {
	...
	mounted: function () { 
		this.$parent.$on('cascadeUpdate', (text) => {
			if( text === 'spellbook'){
				this.$forceUpdate();
			}
		})
		...
		<table>
			<tr v-for="s in viewing" :key="s.id" @mouseenter.capture.stop="itemOver($event, s)">
				...
				<td>
					...
					<button v-if="s.owned && list.canAdd(s)"
						@click="list.add(s); forceRefresh()">Memorize</button>
...
div.spellbook table tr td:nth-child(2) {
	flex: 1;
}
