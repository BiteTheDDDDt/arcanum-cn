...
<template>
  <div class="enchants">
    ...
    <div class="separate">
      <div class="filters">
        ...
        <div class="enchant-list">
          <div class="enchant" v-for="it in usable" :key="it.id" @mouseenter.capture.stop="itemOver($event, it)">
            ...
            <button type="button" v-if="it.buy && !it.owned" :disabled="!it.canBuy(game)" @click="emit('buy', it)">
              🔒
            </button>

            <button  v-else type="button"  @click="begin(it, target)" :disabled="!it.canUse()">
              Enchant
            </button>
          </div>
        </div>
      ...
      <inv :selecting="true" :inv="state.inventory" v-model="target" :types="['armor', 'weapon']" :kinds="viewKinds"
        hide-space="true" />
    </div>
  </div>
</template>
