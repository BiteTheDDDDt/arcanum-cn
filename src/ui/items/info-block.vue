...
export default {
	...
	methods: {
		processItems() {
			...
			this.items = this.effectItems(this.info, this.rate, this.checkType,
				this.target);
		...
		effectItems(obj, rate = false, checkType = null, target = null) {
			...
			if (type === 'number') {
				...
				this.infos.add(
					DisplayName(obj),
					true, false, checkType, InfoBlock.GetItem(obj));

			} else if (Array.isArray(obj)) obj.forEach(
				v => this.effectList(v, '', rate, checkType, null, target));
				...
				[...body.matchAll(GdataIdRegex)].forEach(match => {
					this.effectList(match.groups.id, '', rate, checkType, null, target)
				});
