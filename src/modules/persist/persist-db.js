...
const openDb = ()=>{
	...
	const openRequest = window.indexedDB.open('arcanum', DB_VERSION);
		...
		openRequest.onupgradeneeded = (e)=>{
			...
			Promise.all([
				createStore(db, CHAR_STORE),
				createStore(db, HALL_STORE)
			]).catch((err)=>rej(err));
...
export async function useDB( persist ){
	...
	function deleteChar( charid ) {
		...
		return waitRequest(
			tx.objectStore(CHAR_STORE).delete(charid)
		);
	}
	...
	function saveChar( data, charid ) {
		const tx = this.db.transaction( [CHAR_STORE], 'readwrite');
		return waitRequest(
			tx.objectStore(CHAR_STORE).put(data, charid)
		);
	}
	...
	async function loadChar( charid ){
		...
		const json = await waitRequest(
			tx.objectStore(CHAR_STORE).get(charid)
		);
	...
	function saveHall( data, hid ){
		...
		return waitRequest(
			tx.objectStore(HALL_STORE).put(data, hid)
		);
	}
	...
	async function loadHall( hid ){
		...
		const json = await waitRequest(
			tx.objectStore(HALL_STORE).get(hid)
		);
